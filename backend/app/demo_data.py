...
STUDENTS = [
    ("liuwenqiang", "刘文强", "2024001", "2024级"),
    ("liuzilong",   "刘子龙",   "2024002", "2024级"),
    ("yanglei",     "杨磊",     "2024003", "2024级"),
    ("liufuqiang",  "刘富强",   "2025001", "2025级"),
    ("panyouqi",    "潘有琪",   "2025002", "2025级"),
    ("tongjian",    "童健",     "2025003", "2025级"),
    ("zhangchenghe","张成赫",   "2025004", "2025级"),
    ("yangkang",    "杨康",     "2026001", "2026级"),
    ("xiexundong",  "谢循东",   "2026002", "2026级"),
    ("fengcheng",   "冯成",     "2026003", "2026级"),
]
...
def clear_student(s: Session, uid: int):
    """清掉某学生的项目 / 往返 / 周报 / 问答（含相关附件文件）"""
    from .models import Grade  # noqa: F401 （保持导入完整性）
...
def apply_student_demo(s: Session, teacher, stu, cfg: dict) -> dict:
    """把 DEMO 里某位学生的数据写入库（先清空该学生既有数据）"""
    clear_student(s, stu.id)
    ...
    for back, md, comment in cfg["reports"]:
        rep = WeeklyReport(student_id=stu.id, week=weeks_back(back), content_md=md, created_at=dt(-back * 7 + 1, 20, 15))
        s.add(rep)
        s.commit()
        s.refresh(rep)
        if comment:
    ...
    s.commit()
    return {"项目": 1, "往返": len(cfg["rounds"]), "周报": len(cfg["reports"]), "提问": len(cfg["questions"])}
