...
/**
 * Gargantua 登录页 —— 黑洞引力透镜 + 飞船轨道 + 全息 HUD。
 *
 * 视觉部分原样移植自 gargantua.html：着色器、轨道、粒子、飞船投影都在
 * `gargantua/engine.js` 里逐帧算，这里只负责 React 侧的三件事：
 *  1. 用 ref 把 canvas / 场景 / HUD 的 DOM 节点交给引擎；
 *  2. 把「会驱动 DOM 的状态」做成受控 state（表单、busy、提示、场景标志）；
 *  3. 接上真实登录，让 4700ms 的穿越特效与 API 请求并行跑。
 *
 * 原页面其实**不登录**（源码注释：Deliberately local: credentials are neither
 * sent nor persisted），finish() 只是复位并提示「本地航程模拟完成」。
 * 这里把那套演出改成了真的业务：成功就转场进系统，失败就中断穿越并报错。
 */
...
export default function Login() {
  ...
  useEffect(() => {
    ...
    const onFsChange = () => engineRef.current?.resize()
  ...
  const submit = async (e) => {
    ...
    engineRef.current?.setEngaging(true)   // 起穿越特效
  ...
  return (
    <main ref={sceneRef} className={rootClass} aria-label="沉浸式黑洞场景">
      ...
      <button className={s.fullscreen} type="button" onClick={toggleFullscreen}
        aria-label={document.fullscreenElement ? '退出全屏' : '进入全屏'} title="全屏">
